...
uniform vec3 lightPos = {0.4,0.4,0.4};
...
void main()
{
	...
	vec3 lightDir = normalize(lightPos); // Giving relative positions for now
	...
	fragColour = vec4(eyeDir, 1);//vec4( emissive + ambient + diffuse + specular, alpha);
}
